...
def _build_version_url(component: str, config: Dict[str, object]) -> tuple[Optional[str], Optional[str]]:
    ...
    if version_url and version_url.strip():
        return version_url.strip(), download_url
...
def check_for_update(component: str, current_version: str, timeout: float = 5.0) -> UpdateResult:
    ...
    version_url, download_url = _build_version_url(component, config)
    if not version_url:
        return UpdateResult(None, False, download_url, "Update source not configured.")
    ...
    try:
        ...
        return UpdateResult(None, False, download_url, f"Update check failed: {exc.reason if hasattr(exc, 'reason') else exc}")
    except Exception as exc:
        return UpdateResult(None, False, download_url, f"Update check failed: {exc}")

    if not remote_version:
        return UpdateResult(None, False, download_url, "Update check returned an empty version string.")

    try:
        remote_tuple = _parse_version(remote_version)
        current_tuple = _parse_version(current_version)
    except Exception:
        return UpdateResult(remote_version, False, download_url, "Unable to compare version strings.")

    is_newer = remote_tuple > current_tuple
    return UpdateResult(remote_version, is_newer, download_url, None)
