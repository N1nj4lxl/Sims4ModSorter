...
class Sims4ModSorterApp(tk.Tk):
    ...
    def _handle_update_result(self, result: UpdateResult) -> None:
        ...
        if result.latest_version and result.is_newer:
            ...
            if result.download_url:
                ...
                wants_download = messagebox.askyesno("Update Available", prompt, parent=self)
                if wants_download:
                    ...
                    log_launch_event("mod_sorter", "update-user-ignore", {"download": result.download_url})
            details = [message]
            if result.download_url:
    ...
    def _create_settings_window(self) -> tk.Toplevel:
        ...
        window.transient(self)
        window.configure(padx=16, pady=16, bg=self._theme_cache.get("bg", "#1f2328"))
        window.minsize(720, 520)
        window.columnconfigure(0, weight=1)
        window.rowconfigure(0, weight=1)

        container = ttk.Frame(window, padding=(4, 0, 4, 4))
        ...
        ttk.Label(header, text="Settings", font=("TkDefaultFont", 14, "bold")).grid(row=0, column=0, sticky="w")
        ttk.Button(header, text="Close", command=self.hide_settings).grid(row=0, column=1, sticky="e")
        ...
        ttk.Label(
            general,
            text=(
                "Review the sorter configuration and launch supporting tools. "
                "Plugin enablement is handled by the Mod Manager."
            ),
            wraplength=540,
            justify="left",
        ).grid(row=0, column=0, sticky="ew")

        info_section = ttk.LabelFrame(general, text="Application")
        info_section.grid(row=1, column=0, sticky="ew", pady=(16, 0))
        info_section.columnconfigure(0, weight=1)
        ttk.Label(info_section, text=f"Version: {self.app_version}").grid(row=0, column=0, sticky="w")
        ttk.Label(info_section, text="Update checks run automatically at launch.").grid(row=1, column=0, sticky="w", pady=(6, 0))

        manage_section = ttk.LabelFrame(general, text="Plugin Management")
        manage_section.grid(row=2, column=0, sticky="ew", pady=(16, 0))
        manage_section.columnconfigure(0, weight=1)
        ttk.Label(
            manage_section,
            text=(
                "Enable or disable user plugins with the dedicated Mod Manager. "
                "Changes take effect the next time you launch the sorter."
            ),
            wraplength=520,
            justify="left",
        ).grid(row=0, column=0, sticky="ew")
        ...
        scanning = ttk.Frame(notebook, padding=16)
        scanning.columnconfigure(0, weight=1)
        notebook.add(scanning, text="Scanning")

        scan_toggle = ttk.LabelFrame(scanning, text="Scan Options")
        scan_toggle.grid(row=0, column=0, sticky="ew")
        ...
        ignore_section.grid(row=1, column=0, sticky="ew", pady=(16, 0))
        ...
        preview_section.grid(row=1, column=0, sticky="nsew", pady=(16, 0))
        ...
        plugins_tab = ttk.Frame(notebook, padding=16)
        plugins_tab.columnconfigure(0, weight=1)
        notebook.add(plugins_tab, text="Plugins")

        ttk.Label(
            plugins_tab,
            text="Configure plugin-supplied options here. Enable or disable plugins in the Mod Manager.",
            wraplength=520,
            justify="left",
        ).grid(row=0, column=0, sticky="ew")
    ...
    def _render_theme_preview(self, canvas: tk.Canvas, palette: Dict[str, str], width: int, height: int) -> None:
        ...
        canvas.create_rectangle(0, 0, width, 28, fill=palette["alt"], outline="")
        canvas.create_text(12, 14, text="Header", fill=palette["fg"], font=("TkDefaultFont", 10, "bold"), anchor="w")
        canvas.create_rectangle(12, 38, width - 12, 62, fill=palette["sel"], outline=palette["accent"], width=2)
        canvas.create_text(18, 49, text="Active row", fill=palette["fg"], anchor="w")
        canvas.create_rectangle(12, 66, width - 12, 86, fill=palette["alt"], outline=palette["accent"], width=1)
        canvas.create_text(18, 75, text="Log preview", fill=palette["fg"], anchor="w")
        canvas.create_rectangle(12, 90, 100, 108, fill=palette["sel"], outline=palette["accent"], width=1)
        canvas.create_text(56, 99, text="Button", fill=palette["fg"], anchor="center")
        canvas.create_rectangle(110, 90, width - 12, 108, fill=palette["accent"], outline=palette["accent"], width=1)
        canvas.create_text((110 + width - 12) / 2, 99, text="Accent", fill=palette["fg"], anchor="center")
    ...
    def _update_theme_preview_highlight(self) -> None:
        ...
        for name, canvas in canvases.items():
            ...
            thickness = 3 if name == selected else 2
