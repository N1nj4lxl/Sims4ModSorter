...
class PluginButton:
    button_id: str
    label: str
    callback: Callable[["Sims4ModSorterApp", "ModAPI"], None]
    location: str = "right"
...
class ModAPI:
    ...
    def update_column(
        ...
        *,
        ...
        self._manager.update_column(column_id, heading=heading, width=width, anchor=anchor)
    ...
    def register_button(
        ...
        *,
        ...
        self._manager.register_button(button_id, label, callback, location=location, tooltip=tooltip)
...
class PluginManager:
    def __init__(self, mods_dir: Path, message_bus: Optional[PluginMessageBus] = None) -> None:
        ...
        self.column_overrides: Dict[str, Dict[str, object]] = {}
        self.buttons: Dict[str, PluginButton] = {}
        self.button_order: List[str] = []
    ...
    def update_column(
        ...
        *,
        ...
        if not normalized:
            return
        override = self.column_overrides.setdefault(normalized, {})
        if heading is not None:
            override["heading"] = str(heading)
        if width is not None and width > 0:
            override["width"] = int(width)
        if anchor is not None:
            override["anchor"] = str(anchor)
    ...
    def get_column_overrides(self) -> Dict[str, Dict[str, object]]:
        return {key: dict(value) for key, value in self.column_overrides.items()}
    ...
    def register_button(
        ...
        *,
        ...
        normalized = (button_id or label or "").strip()
        if not normalized or not callable(callback):
            return
        if normalized in self.buttons:
            return
        slot = location.lower().strip() if location else "right"
        if slot not in {"top", "right"}:
            slot = "right"
        self.buttons[normalized] = PluginButton(normalized, label or normalized, callback, slot, tooltip)
        self.button_order.append(normalized)

    def get_buttons(self) -> List[PluginButton]:
        return [self.buttons[key] for key in self.button_order if key in self.buttons]
...
class Sims4ModSorterApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.mod_manager_version = get_local_version("mod_manager")
        ...
        self._plugin_buttons: List[PluginButton] = []
        self._column_overrides: Dict[str, Dict[str, object]] = {}
        if self.plugin_manager:
            self.plugin_manager.attach_app(self)
            self._plugin_columns = self.plugin_manager.get_columns()
            self._plugin_buttons = self.plugin_manager.get_buttons()
            self._column_overrides = self.plugin_manager.get_column_overrides()
        ...
        self._update_window: Optional[tk.Toplevel] = None
        self._update_progress: Optional[ttk.Progressbar] = None
        self._update_check_running = False
        self._manual_update_request = False
    ...
    def _invoke_plugin_button(self, plugin_button: PluginButton) -> None:
        if not plugin_button or not self.plugin_manager:
            return
        api = self.plugin_manager.api
        try:
            plugin_button.callback(self, api)
        except Exception as exc:
            self.log(f"Plugin action '{plugin_button.label}' failed: {exc}")
    ...
    def _start_update_check(self, manual: bool = False) -> None:
        if self._update_check_running:
            if manual:
                ...
                messagebox.showinfo("Update Check", "An update check is already running.", parent=parent)
            return
        self._update_check_running = True
        self._manual_update_request = manual
        version = self.app_version
        if manual:
            log_launch_event("mod_sorter", "update-check-manual", {"source": "settings"})
    ...
    def _show_update_window(self) -> None:
        ...
        window.configure(bg=palette.get("bg", "#2b2b2b"))
        ttk.Label(window, text="Checking for updates...").pack(padx=24, pady=(18, 12))
        progress = ttk.Progressbar(window, mode="indeterminate", length=240)
    ...
    def _handle_update_result(self, result: UpdateResult) -> None:
        manual = self._manual_update_request
        self._manual_update_request = False
        self._update_check_running = False
        self._close_update_window()
        parent = self.settings_window if self.settings_window and self.settings_window.winfo_exists() else self
        if result.message:
            ...
            log_launch_event(
                ...
                {"message": result.message, "manual": manual},
            )
            if manual:
                messagebox.showwarning("Update Check", result.message, parent=parent)
            return
        if result.latest_version and result.is_newer:
            ...
            if result.download_url:
                ...
                prompt = (
                    ...
                    f"You are currently on {self.app_version}.\n\nOpen the download page?"
                )
                wants_download = messagebox.askyesno("Update Available", prompt, parent=parent)
                if wants_download:
                    webbrowser.open(result.download_url)
                    log_launch_event(
                        "mod_sorter",
                        "update-user-launch",
                        {"download": result.download_url, "manual": manual},
                    )
                else:
                    log_launch_event(
                        "mod_sorter",
            ...
            details = [message]
            if result.download_url:
                details.append(f"Download: {result.download_url}")
        ...
        if result.latest_version:
            message = f"Mod Sorter is up to date (version {result.latest_version})."
            self.log(message)
            log_launch_event(
                "mod_sorter",
                "update-current",
                {"version": result.latest_version, "manual": manual},
            )
            if manual:
                messagebox.showinfo("Update Check", message, parent=parent)
        elif manual:
            messagebox.showinfo(
                "Update Check",
                f"Mod Sorter is up to date (version {self.app_version}).",
                parent=parent,
            )
    # ------------------------------------------------------------------
    # UI construction
    ...
    def _build_style(self) -> None:
        ...
        style.map(
            "SettingsNav.TRadiobutton",
            background=[("selected", palette["sel"]), ("active", palette["alt"])],
            foreground=[("selected", palette["bg"]), ("active", palette["fg"])],
        )
    ...
    def _build_ui(self) -> None:
        ...
        for plugin_button in self._plugin_buttons:
            if plugin_button.location != "top":
                continue
            ttk.Button(top, text=plugin_button.label, command=lambda b=plugin_button: self._invoke_plugin_button(b)).pack(
                side="left", padx=4
            )
        ...
        if self._plugin_columns:
            ...
            active_columns: List[PluginColumn] = []
            for plugin_column in self._plugin_columns:
                column_id = plugin_column.column_id
                if not column_id or column_id in columns:
                    continue
                columns.insert(insert_at, column_id)
                insert_at += 1
                active_columns.append(plugin_column)
            self._plugin_columns = active_columns
        ...
        for column in columns:
            override = self._column_overrides.get(column, {})
            heading_text = override.get("heading", headings.get(column, column))
            self.tree.heading(column, text=heading_text)

        def apply_column_config(column: str, *, default_width: Optional[int] = None, default_anchor: Optional[str] = None) -> None:
            override = self._column_overrides.get(column, {})
            width = override.get("width", default_width)
            anchor = override.get("anchor", default_anchor)
        ...
        apply_column_config("inc", default_width=40, default_anchor="center")
        apply_column_config("rel", default_width=220, default_anchor="w")
        apply_column_config("name", default_width=360, default_anchor="w")
        apply_column_config("size", default_width=70, default_anchor="e")
        apply_column_config("type", default_width=170, default_anchor="w")
        apply_column_config("target", default_width=200, default_anchor="w")
        apply_column_config("conf", default_width=60, default_anchor="e")
        for plugin_column in self._plugin_columns:
            override = self._column_overrides.get(plugin_column.column_id, {})
            width = override.get("width", plugin_column.width)
            anchor = override.get("anchor", plugin_column.anchor)
            self.tree.column(plugin_column.column_id, width=width, anchor=anchor, stretch=False)
        apply_column_config("linked", default_width=80, default_anchor="center")
        apply_column_config("meta", default_width=180, default_anchor="w")
        apply_column_config("notes", default_width=260, default_anchor="w")
        ...
        right_buttons = [button for button in self._plugin_buttons if button.location == "right"]
        if right_buttons:
            ttk.Separator(right).pack(fill="x", pady=10)
            action_frame = ttk.LabelFrame(right, text="Plugin Actions")
            action_frame.pack(fill="x", pady=(0, 10))
            for button in right_buttons:
                ttk.Button(
                    action_frame,
                    text=button.label,
                    command=lambda b=button: self._invoke_plugin_button(b),
                ).pack(fill="x", pady=2)
    ...
    def _build_settings_overlay(self) -> None:
        ...
        self.theme_preview_container: Optional[ttk.Frame] = None
        self.theme_preview_canvases: Dict[str, tk.Canvas] = {}
        self._settings_section_var = tk.StringVar(value="general")
        self._settings_section_frames: Dict[str, ttk.Frame] = {}
        self.theme_cb: Optional[ttk.Combobox] = None
    ...
    def _create_settings_window(self) -> tk.Toplevel:
        ...
        window.transient(self)
        window.configure(bg=palette.get("bg", "#1f2328"))
        window.minsize(860, 560)
        window.columnconfigure(0, weight=1)
        window.rowconfigure(0, weight=1)

        container = ttk.Frame(window, padding=(20, 16, 20, 20))
        ...
        ttk.Label(header, text="Sims 4 Mod Sorter", font=("TkDefaultFont", 16, "bold")).grid(row=0, column=0, sticky="w")
        ttk.Label(
            header,
            text="Configure appearance, scanning behaviour, and plugin controls.",
            wraplength=520,
            justify="left",
        ).grid(row=1, column=0, sticky="w", pady=(2, 0))
        ttk.Button(header, text="Close", command=self.hide_settings).grid(row=0, column=1, rowspan=2, sticky="e")

        nav = ttk.Frame(container, padding=(0, 4, 12, 4))
        ...
        self._settings_section_var.set("general")
        section_definitions = [
            ("general", "Overview"),
            ("scanning", "Scanning"),
        ...
        for index, (section_key, section_label) in enumerate(section_definitions):
            button = ttk.Radiobutton(
                nav,
                text=section_label,
                value=section_key,
                variable=self._settings_section_var,
                command=lambda key=section_key: self._show_settings_section(key),
                style="SettingsNav.TRadiobutton",
                takefocus=True,
            )
            button.pack(fill="x", pady=4)
            button.configure(width=18, cursor="hand2")
            if index == 0:
                button.focus_set()

        self._settings_section_frames = {}

        general = ttk.Frame(content)
        general.grid(row=0, column=0, sticky="nsew")
        general.columnconfigure(0, weight=1)
        self._settings_section_frames["general"] = general

        ttk.Label(
            general,
            text="Review version details, launch supporting tools, and trigger update checks.",
            wraplength=560,
            justify="left",
        ).grid(row=0, column=0, sticky="w")

        version_section = ttk.LabelFrame(general, text="Versions & Updates")
        version_section.grid(row=1, column=0, sticky="ew", pady=(16, 0))
        version_section.columnconfigure(1, weight=1)
        ttk.Label(version_section, text="Mod Sorter Version:").grid(row=0, column=0, sticky="w")
        ttk.Label(version_section, text=self.app_version).grid(row=0, column=1, sticky="w")
        ttk.Label(version_section, text="Mod Manager Version:").grid(row=1, column=0, sticky="w", pady=(4, 0))
        ttk.Label(version_section, text=self.mod_manager_version).grid(row=1, column=1, sticky="w", pady=(4, 0))
        ttk.Button(version_section, text="Check For Updates", command=self._manual_update_check).grid(
            row=0, column=2, rowspan=2, padx=(12, 0), sticky="e"
        )

        manage_section = ttk.LabelFrame(general, text="Mod Management")
        manage_section.grid(row=2, column=0, sticky="ew", pady=(16, 0))
        manage_section.columnconfigure(0, weight=1)
        ttk.Label(
            manage_section,
            text="Enable or disable plugins from the Mod Manager. Changes take effect the next time the sorter starts.",
            wraplength=560,
            justify="left",
        ).grid(row=0, column=0, sticky="w")
        ttk.Button(manage_section, text="Open Mod Manager", command=self._open_mod_manager).grid(
            row=1, column=0, sticky="w", pady=(8, 0)
        )

        scanning = ttk.Frame(content)
        scanning.grid(row=0, column=0, sticky="nsew")
        scanning.columnconfigure(0, weight=1)
        self._settings_section_frames["scanning"] = scanning

        ttk.Label(
            scanning,
            text="Adjust how the scan treats folders and file patterns.",
            wraplength=560,
            justify="left",
        ).grid(row=0, column=0, sticky="w")

        scan_toggle = ttk.LabelFrame(scanning, text="Scan Behaviour")
        scan_toggle.grid(row=1, column=0, sticky="ew", pady=(16, 0))
        ttk.Checkbutton(scan_toggle, text="Scan subfolders", variable=self.recurse_var).grid(row=0, column=0, sticky="w")
        ttk.Label(
            scan_toggle,
            text="Toggle whether nested directories are examined during each scan.",
            wraplength=520,
            justify="left",
        ).grid(row=1, column=0, sticky="w", pady=(4, 0))

        ignore_section = ttk.LabelFrame(scanning, text="Ignore Filters")
        ignore_section.grid(row=2, column=0, sticky="ew", pady=(16, 0))
        ignore_section.columnconfigure(0, weight=1)
        ttk.Label(ignore_section, text="Ignore extensions (comma separated)").grid(row=0, column=0, sticky="w")
        ttk.Entry(ignore_section, textvariable=self.ignore_exts_var).grid(row=1, column=0, sticky="ew", pady=(2, 0))
        ttk.Label(ignore_section, text="Ignore names containing (comma separated)").grid(
            row=2, column=0, sticky="w", pady=(12, 0)
        )
        ttk.Entry(ignore_section, textvariable=self.ignore_names_var).grid(row=3, column=0, sticky="ew", pady=(2, 0))

        appearance = ttk.Frame(content)
        appearance.grid(row=0, column=0, sticky="nsew")
        appearance.columnconfigure(0, weight=1)
        self._settings_section_frames["appearance"] = appearance

        ttk.Label(
            appearance,
            text="Choose a theme and preview how the interface updates before applying it.",
            wraplength=560,
            justify="left",
        ).grid(row=0, column=0, sticky="w")

        theme_controls = ttk.LabelFrame(appearance, text="Theme Selection")
        theme_controls.grid(row=1, column=0, sticky="ew", pady=(16, 0))
        ...
        self.theme_cb.grid(row=0, column=1, sticky="ew", padx=(8, 8))
        ttk.Button(theme_controls, text="Apply", command=self.on_apply_theme).grid(row=0, column=2, sticky="e")

        preview_section = ttk.LabelFrame(appearance, text="Theme Previews")
        preview_section.grid(row=2, column=0, sticky="nsew", pady=(16, 0))
        preview_section.columnconfigure(0, weight=1)
        preview_section.rowconfigure(0, weight=1)
        self.theme_preview_container = ttk.Frame(preview_section)
        self.theme_preview_container.grid(row=0, column=0, sticky="nsew")
        for column in range(2):
            self.theme_preview_container.columnconfigure(column, weight=1, uniform="preview")
        self._build_theme_preview_widgets()

        plugins_tab = ttk.Frame(content)
        plugins_tab.grid(row=0, column=0, sticky="nsew")
        plugins_tab.columnconfigure(0, weight=1)
        self._settings_section_frames["plugins"] = plugins_tab

        ttk.Label(
            plugins_tab,
            text="Plugin authors can expose settings below. Use the Mod Manager to control activation.",
            wraplength=560,
            justify="left",
        ).grid(row=0, column=0, sticky="w")

        if self.plugin_manager and self.plugin_manager.settings_sections:
            sections_frame = ttk.Frame(plugins_tab)
            sections_frame.grid(row=1, column=0, sticky="nsew", pady=(16, 0))
            sections_frame.columnconfigure(0, weight=1)
            for index, (title, builder) in enumerate(self.plugin_manager.settings_sections):
                panel = ttk.LabelFrame(sections_frame, text=title)
            ...
            ttk.Label(plugins_tab, text="No plugin settings are available.").grid(
                row=1, column=0, sticky="w", pady=(12, 0)
            )

        self._show_settings_section(self._settings_section_var.get())

        window.bind("<Escape>", lambda _e: self.hide_settings())
    ...
    def _show_settings_section(self, name: str) -> None:
        frames = getattr(self, "_settings_section_frames", {})
        if name not in frames:
            if "general" in frames:
                name = "general"
            elif frames:
        ...
        for key, frame in frames.items():
            try:
                ...
                continue
        if self._settings_section_var.get() != name:
            self._settings_section_var.set(name)
    # ------------------------------------------------------------------
    # Theme preview helpers
    # ------------------------------------------------------------------
    def _build_theme_preview_widgets(self) -> None:
        container = self.theme_preview_container
        if container is None:
            return
        for child in container.winfo_children():
            child.destroy()
        self.theme_preview_canvases = {}
        columns = 2
        width, height = 220, 140
        for index, (name, palette) in enumerate(THEMES.items()):
            row = index // columns
            column = index % columns
            cell = ttk.Frame(container, padding=8)
            cell.grid(row=row, column=column, padx=6, pady=6, sticky="nsew")
            cell.columnconfigure(0, weight=1)
            container.rowconfigure(row, weight=1)
            border_color = self._theme_cache.get("alt", "#4a4a4a")
            canvas = tk.Canvas(
                cell,
                width=width,
                height=height,
            ...
            self._render_theme_preview(canvas, palette, width, height)
            label = ttk.Label(cell, text=name, anchor="center", font=("TkDefaultFont", 10, "bold"))
            label.grid(row=1, column=0, pady=(8, 0))
            swatch = ttk.Label(
                cell,
                text=f"Accent {palette['accent']}  •  Highlight {palette['sel']}",
                anchor="center",
                justify="center",
            )
            swatch.grid(row=2, column=0, pady=(2, 0))
    ...
    def _render_theme_preview(self, canvas: tk.Canvas, palette: Dict[str, str], width: int, height: int) -> None:
        canvas.create_rectangle(0, 0, width, height, fill=palette["bg"], outline="")
        canvas.create_rectangle(4, 4, width - 4, height - 4, fill=palette["bg"], outline=palette["alt"], width=1)
        canvas.create_rectangle(4, 4, width - 4, 36, fill=palette["alt"], outline="")
        canvas.create_text(16, 20, text="Dashboard", fill=palette["fg"], font=("TkDefaultFont", 10, "bold"), anchor="w")
        canvas.create_rectangle(width - 84, 10, width - 16, 30, fill=palette["accent"], outline="")
        canvas.create_text(width - 50, 20, text="Status", fill=palette["fg"], font=("TkDefaultFont", 9, "bold"))
        ...
        for idx in range(3):
            ...
            if idx == 0:
                ...
                outline = palette["alt"]
                text = "Disabled"
            canvas.create_rectangle(16, y0, width - 16, y1, fill=fill, outline=outline, width=1)
            canvas.create_text(24, y0 + 11, text=text, fill=palette["fg"], anchor="w", font=("TkDefaultFont", 9))

        canvas.create_rectangle(16, height - 40, width - 120, height - 18, fill=palette["sel"], outline=palette["accent"], width=1)
        canvas.create_text((16 + width - 120) / 2, height - 29, text="Primary", fill=palette["fg"], font=("TkDefaultFont", 9, "bold"))
        canvas.create_rectangle(width - 112, height - 40, width - 16, height - 18, fill=palette["accent"], outline=palette["accent"], width=1)
        canvas.create_text(width - 64, height - 29, text="Accent", fill=palette["fg"], font=("TkDefaultFont", 9, "bold"))
    ...
    def _open_mod_manager(self) -> None:
        manager_path = Path(__file__).with_name("mod_manager.py")
        try:
            subprocess.Popen([sys.executable, str(manager_path)])
        except Exception as exc:
            self.log(f"Mod Manager launch failed: {exc}")
            messagebox.showerror("Mod Manager", f"Unable to launch Mod Manager: {exc}", parent=self)

    def _on_theme_preview_click(self, theme_name: str) -> None:
        self.theme_name.set(theme_name)
        combo = getattr(self, "theme_cb", None)
    ...
    def _update_theme_preview_highlight(self) -> None:
        ...
        for name, canvas in canvases.items():
            ...
            thickness = 4 if name == selected else 2
            canvas.configure(highlightbackground=border, highlightcolor=border, highlightthickness=thickness)
    # ------------------------------------------------------------------
    ...
    def hide_settings(self) -> None:
        ...
        self.theme_preview_container = None
        self.theme_preview_canvases = {}
        self._settings_section_frames = {}
        self.theme_cb = None
    ...
    def _on_resize(self, _event: Optional[tk.Event] = None) -> None:
        ...
        static_columns = [col for col in self._column_order if col not in {"name", "notes"}]
        fixed = 0
        for column in static_columns:
            try:
                fixed += int(self.tree.column(column, option="width") or 0)
            except tk.TclError:
                continue
